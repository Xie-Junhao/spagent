...
class CropTool(Tool):
    ...
    def _result(
        ...
        if single:
            extras.update({key: value for key, value in crops[0].items() if key != "output_path"})
        return ToolResult(
            success=True,
            payload=payload,
