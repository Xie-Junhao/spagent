...
> The **Tool Name** column is the *registered* name (`ToolCatalogEntry.tool_name` in `spagent/tools/catalog.py`) exposed to the model — not the Python class or filename. Generation tools use capability-specific names such as `image_generation_sana_tool`, `qwen_image_edit_tool`, and `video_generation_{veo,sora,wan,vace}_tool`.
...
| **qwen_image_edit_tool** | Image Generation / Editing | Edit an existing image from an instruction or fuse content from multiple images using Qwen Image. | **image_path** (required, string): Base image | **reference_image_paths** (optional, up to 2): Reference content/style images | **prompt** (required, string): Editing instruction<br>**negative_prompt** (optional, max 500 chars) | **size** (optional, `width*height`)<br>**n** (optional, default=1, range 1-6)<br>**seed** (optional)<br>**prompt_extend** (optional, default=true)<br>**watermark** (optional, default=false) | ✅ **output_path** (string): Primary edited PNG<br>✅ **image_paths** (list): All edited images<br>✅ **source_image_paths**, **model**, **size**, **seed**, **request_id** | ⚠️ Region masks and local-model deployment |
...
| **wilddet3d_tool** | Detection (3D) | Promptable 3D object detection from a single RGB image. | **image_path** (required, string) | ❌ None | **text_prompt** (one prompt mode required, string): comma- or period-separated object categories | **boxes** (optional, 1–20 pixel-xyxy boxes)<br>**points** (optional, up to 100 `[x,y,label]` points, including a foreground point)<br>**score_threshold** (optional, default=0.3)<br>Legacy `prompt_text` / `input_boxes` / `input_points` call kwargs remain accepted | ✅ Pixel-xyxy **boxes_2d**, labels, scores<br>✅ **boxes_3d** as `[center_xyz(3), dimensions(3), quaternion_wxyz(4)]`<br>✅ depth and annotated image paths<br>✅ legacy `boxes2d` / `boxes3d` aliases | ⚠️ 3D mesh generation, relative spatial positions |
