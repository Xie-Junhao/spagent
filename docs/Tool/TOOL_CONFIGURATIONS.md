...
| **infinidepth_tool** | Depth Estimation | Estimate high-resolution relative depth from one RGB image with InfiniDepth. | **image_path** (required, string) | ❌ None | ❌ None | **output_resolution_mode** (optional, default=`original`, enum: `original`/`upsample`)<br>**upsample_ratio** (optional, default=2, 1–4; used in upsample mode)<br>**save_pcd** (optional, default=false) | ✅ **depth_path** / **colored_depth_path**<br>✅ actual **shape** and input **source_shape**<br>✅ optional **point_cloud_path** | ⚠️ Raw float depth array, confidence/uncertainty map, camera intrinsics |
